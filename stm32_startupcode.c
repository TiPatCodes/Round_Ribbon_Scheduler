...
uint32_t __attribute__((section(".isr_vector"))) vectors[]  =
{
    ...
    (uint32_t)NMI_Handler,  // because we have used the "alias" attribute when we refer to the address of NMI it will take the address of default handler 
    (uint32_t)HardFault_Handler,
    // like wise we will create each Handler address space in the vector table 
...
void Reset_Handler(void)
{
    ...
    uint8_t *pDst_data = (uint8_t*)_sdata ;  // SRAM1
    ...
    uint8_t *pDst = (uint8_t*)_sbss ;  // SRAM1 start of the .bss
